"""Debug CLI (Phase 18): chains the FULL pipeline through Phase 17
(FrameSource -> ... -> EvidenceBuilder -> evidence_service -> Reasoner ->
decision_service, all constructed ONCE before the loop) against
people_clip.mp4, extended with a REAL run_verification_if_warranted() call
after each persisted DecisionResult — genuine end-to-end construction, not
simulated.

RESOLUTION (cadence is NOT this phase's decision, same as Phase 16/17):
calling run_verification_if_warranted after EVERY decision here is a
REASONABLE cadence for DEMONSTRATION PURPOSES ONLY — should_verify()
itself still gates on outcome==INCIDENT internally (Decision D), so most
calls here are genuinely no-ops (applicable=False), same as production
would see.

======================================================================
KNOWN ENVIRONMENT CAVEAT — same class of issue as Phase 13-17
======================================================================
...
ACTIVELY CONFIGURED values at startup and warns loudly if they differ.
...
persist -> Verifier (if outcome==INCIDENT) -> persist cycle, guaranteeing
at least one real, genuinely verified INCIDENT decision.

Usage: python scripts/preview_verifier.py <video_id>
"""
...
from app.services import decision_service, evidence_service, session_service, verification_service  # noqa: E402
...
def _print_active_config_diagnostic() -> None:
    ...
    print(
        f"OLLAMA_BASE_URL={settings.OLLAMA_BASE_URL!r} "
        f"VERIFIER_REQUEST_TIMEOUT_SECONDS={settings.VERIFIER_REQUEST_TIMEOUT_SECONDS}"
    )
...
def _run_cycle(
    ...
    # Phase 18: severity-gated verification (Decision D — should_verify()
    # itself decides applicability, gated internally on outcome==INCIDENT).
    try:
        start = time.perf_counter()
    ...
    if not verification_outcome.applicable:
        print(f"    verification: NOT APPLICABLE (outcome={decision_result.outcome.value} != INCIDENT)")
        return

    latencies.append(("verifier", verification_latency))
    verifications.append(verification_outcome)
    result = verification_outcome.verification_result
    print(
        f"    verification: APPLIED, real_latency={verification_latency:.2f}s "
        f"passed={result.passed} confidence_ok={result.confidence_consistency_ok} "
        f"grounding_ok={result.evidence_grounding_existence_ok}"
    )
    if result.llm_checks is not None:
        print(
            f"      llm_checks: reasoning_consistency_ok={result.llm_checks.reasoning_consistency_ok} "
            f"contradiction_handling_ok={result.llm_checks.contradiction_handling_ok} "
            f"recommendation_consistency_ok={result.llm_checks.recommendation_consistency_ok} "
            f"unsupported_claims_found={result.llm_checks.unsupported_claims_found} "
            f"overall_verdict={result.llm_checks.overall_verdict}"
        )
    if result.issues_found:
        print(f"      issues_found={result.issues_found}")
    if verification_outcome.superseding_decision is not None:
        print(
            f"      SUPERSEDED by new ABSTAIN decision_id={verification_outcome.superseding_decision.id} "
            f"(original decision_id={decision_result.decision_id} left UNCHANGED)"
        )


def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python scripts/preview_verifier.py <video_id>")
        sys.exit(1)
    ...
    try:
        ...
        print(f"FATAL: Verifier unavailable: {exc}")
        db.close()
        sys.exit(1)
        ...
        for frame in source.frames():
            ...
            if prev_frame is not None:
                ...
                if trigger_decision.trigger_type != TriggerType.NONE:
                    ...
                    _run_cycle(
                        "REAL", db, session.id, builder, vision_model, reasoner, verifier, frame, crowd_metrics,
                        risk_result, trigger_decision, roi_bbox, compact_metrics,
                        built_packages, built_decisions, failures, latencies, verifications,
                    )
    ...
    # ==================================================================
    # SYNTHETIC STRESS-TEST ADDENDUM — forces a trigger if none fired, AND
    # guarantees at least one INCIDENT outcome so should_verify() actually
    # gates something True (not just no-ops the whole run)
    # ==================================================================
    if total_real_triggers == 0:
        ...
        print(
            ...
            "persist -> Reasoner -> persist -> Verifier -> persist cycle."
        )
        ...
        for stage_name, stage_value in (("ELEVATED", elevated_value), ("CRITICAL", critical_value)):
            for _ in range(stage_frames):
                ...
                if trigger_decision.trigger_type != TriggerType.NONE:
                    ...
                    _run_cycle(
                        "SYNTHETIC", db, session.id, builder, vision_model, reasoner, verifier, last_real_frame,
                        synthetic_crowd_metrics, risk_result, trigger_decision, roi_bbox,
                        compact_metrics, built_packages, built_decisions, failures, latencies, verifications,
                    )
                    synthetic_cycles += 1
    ...
    print(f"Total verification cycles applied (outcome==INCIDENT): {len(verifications)}")
    verifier_latencies = [v for k, v in latencies if k == "verifier"]
    if verifier_latencies:
        print(
            f"Real Verifier latency: {[round(v, 2) for v in verifier_latencies]} "
            f"(max={max(verifier_latencies):.2f}s, configured timeout={settings.VERIFIER_REQUEST_TIMEOUT_SECONDS}s)"
        )
    if failures:
